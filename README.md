...
**1.10.0** — feature release in two threads. **Protocol compatibility:** compressed envelopes (`Content-Encoding: gzip / deflate / zstd`, decompressed size bounded by `CRASHBOX_MAX_ENVELOPE_BYTES` — zip-bomb safe), the legacy `POST /api/:id/store/` endpoint for pre-envelope SDKs (opt-in via `CRASHBOX_ENABLE_LEGACY_STORE_ENDPOINT`), and `X-Sentry-Rate-Limits` + `Retry-After` on 429 so SDKs back off properly. **Self-hosted operations:** `GET /api/admin/backup` streams an atomic SQLite snapshot (`VACUUM INTO`; works with read-scope tokens — one `curl` in a cron line is a full backup strategy), declarative heartbeat provisioning via `CRASHBOX_HEARTBEAT_MONITORS` (fixed ping keys survive container recreation; idempotent, UI edits respected), and an opt-in periodic **digest** (`CRASHBOX_DIGEST_*`) summarizing new issues / events / top offenders per project through the existing notification channels — silent when there's nothing to say. See [`docs/protocol.md`](docs/protocol.md), [`docs/heartbeats.md`](docs/heartbeats.md), [`docs/api.md`](docs/api.md).
