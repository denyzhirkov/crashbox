...
docker run -d \
  ...
  denyzhirkov/crashbox:1.0.0
```

Image: [`denyzhirkov/crashbox`](https://hub.docker.com/r/denyzhirkov/crashbox) on Docker Hub (40 MB, distroless, non-root). Pin to a specific tag (`:1.0.0`) in production; `:latest` follows the most recent release.
...
**1.0.0** — first public release. Single-tenant, single-user, single binary. All acceptance criteria from the original brief are met (see [acceptance check](#acceptance) below). Published to Docker Hub as [`denyzhirkov/crashbox`](https://hub.docker.com/r/denyzhirkov/crashbox). Use at your own risk on something small first.
